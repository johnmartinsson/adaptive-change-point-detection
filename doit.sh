...
# TODO: automate, or explain BirdNET-Analyzer installation
...
n_runs=5
results_dir=results/${n_runs}_runs/
n_queries_budget=$2
noise=$1
...
echo "start time: $(date)" >> ${results_dir}/${settings}_time.txt
...
do
    ...
    do        
        bash ./scripts/evaluate_all.sh $class_name ${results_dir}/$class_name/n_queries_${n_queries_budget}_noise_${noise}/ $model_name $base_dir $n_runs
    done
done
...
do
    for model_name in mlp prototypical
    do        
        python sound_event_eval.py --class_name=$class_name --model_name=$model_name --t_collar=$t_collar --n_runs=$n_runs --sim_dir=${results_dir}/$class_name/n_queries_${n_queries_budget}_noise_${noise}/ --base_dir=$base_dir
    done
done
